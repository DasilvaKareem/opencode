...
// 2. USER_CONFIG/playscape/themes/*.json
// 3. PROJECT_ROOT/.playscape/themes/*.json
// 4. CWD/.playscape/themes/*.json
func LoadThemesFromDirectories(userConfig, projectRoot, cwd string) error {
	...
	dirs := []string{
		...
		filepath.Join(projectRoot, ".playscape", "themes"),
	}
	if cwd != projectRoot {
		dirs = append(dirs, filepath.Join(cwd, ".playscape", "themes"))
	}
