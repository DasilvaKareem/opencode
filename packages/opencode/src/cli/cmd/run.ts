...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run Playscape with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
  ...
  handler: async (args) => {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      if (cfg.share === "auto" || Flag.PLAYSCAPE_AUTO_SHARE || args.share) {
        try {
          await Session.share(session.id)
          UI.println(UI.Style.TEXT_INFO_BOLD + "~  https://playscape.ai/s/" + session.id.slice(-8))
        } catch (error) {
